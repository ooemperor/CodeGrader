...
"""
Handler Class for the Settings View
"""
...
class Settingshandler(BaseHandler):
    ...
    def get(self) -> Union[str, Response]:
        ...
        @rtype: str|Response
        """
        settings = self.api.get(f"/user/{self.user.id}")
        submissions = self.api.get(f"/submissions", user_id=self.user.id)
        if "submission" in submissions.keys():  # handles the case that there are no submissions yet
            settings["submissions"] = submissions["submission"]
